...
import { WarningCircle, Broom, MagnifyingGlass, CheckCircle, Warning, UploadSimple, XCircle, Package } from '@phosphor-icons/react';
...
export default function TabCleaning({ room, roomId, onRefreshRoom }) {
  // 🔥 Lỗi 2 Key trùng nhau đã được FIX: Chỉ khai báo 1 Context duy nhất
  ...
  // 🔥 Tách riêng hàm fetch để dùng chung cho cả UseEffect và SignalR
  const fetchInventories = async (isRealtime = false) => {
    ...
    try {
      ...
      if (!isRealtime) api.error({ placement: 'bottomRight', title: 'Lỗi', description: 'Không thể tải danh sách.' });
    } finally {
  ...
  // 🔥 Đã thêm LẮNG NGHE REALTIME: Tự động âm thầm làm mới danh sách
  useSignalR(() => {
  ...
  const handleUpdateCleaning = async (newCleaning) => {
    try {
      ...
      api.success({ placement: 'bottomRight', title: 'Thành công', description: 'Đã cập nhật tiến độ cho Lễ tân!' });
      onRefreshRoom();
    } catch (e) { 
      api.error({ placement: 'bottomRight', title: 'Lỗi', description: 'Không thể cập nhật trạng thái.' }); 
    } 
  ...
  const submitDamageReport = async (values) => {
    try {
      ...
      api.success({ placement: 'bottomRight', title: 'Thành công', description: 'Biên bản sự cố đã gửi về Lễ Tân!' });
      setIsReportModalOpen(false);
    } catch (error) {
      api.error({ placement: 'bottomRight', title: 'Lỗi báo cáo', description: error.response?.data?.message || 'Có lỗi xảy ra' });
    } finally { setLoading(false); }
  };
  ...
  const handleCancelReport = async (inventoryId, reportId) => {
    try {
      ...
      api.success({ placement: 'bottomRight', title: 'Thành công', description: 'Đã thu hồi báo cáo sự cố!' });
    } catch (error) {
      api.error({ placement: 'bottomRight', title: 'Lỗi', description: 'Không thể hủy báo cáo lúc này.' });
    } finally {
      setLoading(false);
    }
  };

  // 🔥 GỌI API YÊU CẦU BỔ SUNG ĐỒ
  const handleRequestRefill = async (item) => {
    try {
      setLoading(true);
      await roomInventoryApi.requestRefill(item.id);
      api.success({ 
        placement: 'bottomRight', 
        title: 'Đã gửi yêu cầu', 
        description: `Đã báo bộ phận Kho bổ sung [${item.equipmentName}] cho phòng này.` 
      });
    } catch (error) {
      api.error({ placement: 'bottomRight', title: 'Lỗi', description: 'Không thể gửi yêu cầu lúc này.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{paddingTop: 12}}>
      ...
      <Card variant="borderless" style={{ background: COLORS.LIGHTEST, marginBottom: 24, borderRadius: 12, textAlign: 'center', padding: '16px 8px' }}>
        ...
        <Button 
          type="primary"
          onClick={() => handleUpdateCleaning(nextState.next)} 
        ...
        <Table 
          ...
          columns={[
            ...
            { 
              ...
              align: 'center', 
              width: 300, 
              render: q => <Tag color={q > 0 ? "blue" : "error"} style={{ margin: 0, padding: '2px 12px', fontSize: 14, fontWeight: 'bold' }}>{q || 0}</Tag> 
            },
            { 
              title: 'Hành động', 
              key: 'action', 
              ...
              render: (_, record) => {
                ...
                if (isOutOfStock) {
                  return (
                    <Button type="dashed" icon={<Package size={18} />} onClick={() => handleRequestRefill(record)} style={{ color: COLORS.MIDNIGHT_BLUE, borderColor: COLORS.MIDNIGHT_BLUE, fontWeight: 600, borderRadius: 6 }}>
                      Yêu cầu bổ sung
                    </Button>
                  );
                }
                ...
                return isReported ? (
                  <Popconfirm title="Bạn có chắc muốn hủy báo cáo này?" onConfirm={() => handleCancelReport(record.id, isReported)} okText="Đồng ý" cancelText="Đóng">
                    <Button style={{ color: COLORS.ACCENT_RED, borderColor: COLORS.ACCENT_RED, background: '#fff', fontWeight: 600, borderRadius: 6 }} icon={<XCircle size={18} />}>
                      Hủy báo cáo
                    </Button>
                  </Popconfirm>
                ) : (
                  <Button type="primary" icon={<WarningCircle size={18} />} onClick={() => { setReportItem(record); reportForm.resetFields(); setFileList([]); setIsReportModalOpen(true); }} style={{ backgroundColor: COLORS.ACCENT_RED, borderColor: COLORS.ACCENT_RED, color: '#fff', fontWeight: 600, borderRadius: 6 }}>
                    Báo hỏng / Mất
                  </Button>
                );
              }
          ...
          {filteredInventories.map(item => {
            ...
            return (
              <div key={item.id} style={{ background: '#fff', border: `1px solid ${COLORS.LIGHTEST}`, borderRadius: 10, padding: 16, boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                ...
                {isOutOfStock ? (
                  <Button block type="dashed" icon={<Package size={18} weight="bold" />} onClick={() => handleRequestRefill(item)} style={{ color: COLORS.MIDNIGHT_BLUE, borderColor: COLORS.MIDNIGHT_BLUE, fontWeight: 600, height: 40, borderRadius: 8, fontSize: 14 }}>
                    Yêu cầu kho bổ sung
                  </Button>
                ) : isReported ? (
                  <Popconfirm title="Thu hồi báo cáo sự cố này?" onConfirm={() => handleCancelReport(item.id, isReported)} okText="Đồng ý" cancelText="Đóng">
                    <Button block icon={<XCircle size={18} />} style={{ color: COLORS.ACCENT_RED, borderColor: COLORS.ACCENT_RED, background: '#fff', fontWeight: 600, height: 40, borderRadius: 8, fontSize: 14 }}>
                      Hủy báo cáo
                    </Button>
                  </Popconfirm>
                ) : (
                  <Button type="primary" block icon={<WarningCircle size={18} weight="bold" />} onClick={() => { setReportItem(item); reportForm.resetFields(); setFileList([]); setIsReportModalOpen(true); }} style={{ backgroundColor: COLORS.ACCENT_RED, borderColor: COLORS.ACCENT_RED, color: '#fff', fontWeight: 600, height: 40, borderRadius: 8, fontSize: 14 }}>
                    Báo hỏng / Mất
                  </Button>
                )}
              </div>
