...
import { Card, Table, Button, Typography, Space, Row, Col, notification, Modal, Form, Input, DatePicker, Tag, Divider, Popconfirm } from 'antd';
        ...
        <Row align="middle" justify="space-between">
          <Col>
            <Space size="large" align="start">
              <div style={{ backgroundColor: currentShift ? '#e6f7ff' : '#f5f5f5', padding: 20, borderRadius: '50%' }}>
                <Clock size={40} color={currentShift ? '#1890ff' : '#bfbfbf'} weight={currentShift ? "fill" : "regular"} />
              </div>
              <div>
                <Title level={4} style={{ margin: 0, color: currentShift ? '#1890ff' : '#8c8c8c' }}>
            ...
            <Space size="middle">
              {!currentShift ? (
                <Popconfirm title="Xác nhận vào ca làm việc bây giờ?" onConfirm={handleCheckIn} okText="Đồng ý" cancelText="Hủy">
                  <Button size="large" type="primary" icon={<SignIn size={20} />} loading={loading} style={{ backgroundColor: SUCCESS_GREEN, height: 50, borderRadius: 8, fontWeight: 'bold' }}>
                    CHECK-IN VÀO CA
                  </Button>
                </Popconfirm>
              ) : (
                <>
                  <Button size="large" icon={<NotePencil size={20} />} onClick={() => setIsHandoverModalOpen(true)} style={{ height: 50, borderRadius: 8, color: MIDNIGHT_BLUE, borderColor: MIDNIGHT_BLUE }}>
                    Viết Bàn Giao
                  </Button>
                  <Popconfirm title="Bạn có chắc muốn kết thúc ca làm việc?" onConfirm={handleCheckOut} okText="Đồng ý" cancelText="Hủy" placement="bottomRight">
                    <Button size="large" type="primary" danger icon={<SignOut size={20} />} loading={loading} style={{ height: 50, borderRadius: 8, fontWeight: 'bold' }}>
                      CHECK-OUT RA CA
                    </Button>
                  </Popconfirm>
                </>
      ...
      >
        ...
        <Form form={handoverForm} layout="vertical" onFinish={onHandoverSubmit}>
          ...
          <div style={{ textAlign: 'right', marginTop: 24 }}>
            <Space>
              <Button onClick={() => setIsHandoverModalOpen(false)}>Hủy</Button>
              <Button type="primary" htmlType="submit" loading={loading} style={{ backgroundColor: ACCENT_RED }}>Lưu Biên Bản</Button>
            </Space>
          </div>
        </Form>
      </Modal>
