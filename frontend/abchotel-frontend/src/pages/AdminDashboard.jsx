...
import { Card, Row, Col, Typography, Statistic, Table, Tag, Progress, Timeline, Space, Grid, Button, notification, Rate, List, Avatar } from 'antd';
import { 
  Bed, Key, WarningCircle, CalendarCheck, ClockClockwise, Users, ArrowRight, Wallet, ArrowsClockwise, Star
...
export default function AdminDashboard() {
  ...
  const [dashboardData, setDashboardData] = useState({
    ...
    recentBookings: [],
    activities: [],
    recentReviews: [] // 🔥 THÊM STATE CHO REVIEWS
  });
  ...
  const fetchDashboard = async () => {
    try {
      ...
      setDashboardData({
        ...
        recentReviews: data.recentReviews || data.RecentReviews || [] // 🔥 LẤY DATA REVIEWS TỪ BACKEND
      });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, paddingBottom: 24, animation: 'fadeIn 0.5s ease-in' }}>
      {contextHolder}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16 }}>
        ...
        <Button 
          type="primary" size="large" icon={<ArrowsClockwise size={20} />} 
          onClick={fetchDashboard} loading={loading} style={{ backgroundColor: COLORS.MIDNIGHT_BLUE }}
        >
          Làm mới
        </Button>
      </div>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card variant="borderless" style={{ borderRadius: 16, background: 'linear-gradient(135deg, #1e3c72 0%, #2a5298 100%)', color: '#fff', boxShadow: '0 4px 12px rgba(30,60,114,0.2)' }}>
            <Statistic 
              ...
              formatter={val => new Intl.NumberFormat('vi-VN').format(val)} suffix="₫"
      ...
      <Row gutter={[24, 24]}>
        <Col xs={24} xl={16}>
          ...
          >
            <Table 
              ...
              pagination={false} loading={loading} scroll={{ x: 600 }}
              columns={[
                { title: 'Mã Đặt', render: (_, record) => <Text strong>{record.id || record.Id}</Text> },
        ...
        <Col xs={24} xl={8}>
          <Card 
            ...
            variant="borderless" style={{ borderRadius: 16, boxShadow: '0 2px 10px rgba(0,0,0,0.03)', height: '100%' }}
          >
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              <Progress 
                ...
                strokeColor={COLORS.MIDNIGHT_BLUE} size={180}
                format={percent => (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Text strong>{dashboardData.roomStatus.occupied || dashboardData.roomStatus.Occupied || 0}</Text>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Text><Badge color="green" /> Trống (Available)</Text>
                <Text strong>{dashboardData.roomStatus.available || dashboardData.roomStatus.Available || 0}</Text>
              </div>
      ...
      {/* 4. KHU VỰC DƯỚI CÙNG: HOẠT ĐỘNG & REVIEWS */}
      <Row gutter={[24, 24]}>
        
        {/* Nhật ký hoạt động*/}
        <Col xs={24} lg={15}>
          <Card 
            title={<Space><ClockClockwise size={22} color={COLORS.MIDNIGHT_BLUE}/> <span style={{fontSize: 18, color: COLORS.MIDNIGHT_BLUE}}>Hoạt động gần đây</span></Space>} 
            variant="borderless" style={{ borderRadius: 16, boxShadow: '0 2px 10px rgba(0,0,0,0.03)', height: '100%' }}
          >
            <Timeline
        ...
        {/* ĐÁNH GIÁ CHƯA DUYỆT TỪ KHÁCH HÀNG*/}
        <Col xs={24} lg={9}>
          <Card 
            title={<Space><Star size={22} color={COLORS.ACCENT_RED} weight="fill"/> <span style={{fontSize: 18, color: COLORS.MIDNIGHT_BLUE}}>Đánh giá cần xử lý</span></Space>} 
            variant="borderless" style={{ borderRadius: 16, boxShadow: '0 2px 10px rgba(0,0,0,0.03)', height: '100%' }}
            extra={<a href="/admin/reviews" style={{ color: COLORS.ACCENT_RED, fontWeight: 600 }}>Xem tất cả <ArrowRight /></a>}
          >
            {dashboardData.recentReviews.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 0', color: COLORS.MUTED }}>
                    Không có đánh giá nào đang chờ xử lý 🎉
                </div>
            ) : (
                <List
                  ...
                  renderItem={item => (
                    <List.Item>
                      <List.Item.Meta
                        avatar={<Avatar style={{ backgroundColor: COLORS.MIDNIGHT_BLUE }}>{(item.customerName || item.CustomerName)?.charAt(0)}</Avatar>}
                        title={
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Text strong>{item.customerName || item.CustomerName}</Text>
                            <Rate disabled defaultValue={item.rating || item.Rating} style={{ fontSize: 12, color: '#faad14' }} />
                          </div>
                        }
                        description={
                          <div>
                            <Text style={{ fontStyle: 'italic', color: COLORS.DARKEST }}>"{item.comment || item.Comment}"</Text>
                            <br/>
                            <Text type="secondary" style={{ fontSize: 12 }}>{dayjs(item.date || item.Date).format('HH:mm DD/MM/YYYY')}</Text>
                          </div>
                        }
                      />
                    </List.Item>
