...
{
    // =========================================================================
    // 1. THÊM BỘ ĐỌC HTTP CONTEXT (ĐỂ LẤY ID USER THẬT TỪ TOKEN JWT)
    // =========================================================================
    ...
    public HotelDbContext(DbContextOptions<HotelDbContext> options, IHttpContextAccessor httpContextAccessor = null)
        : base(options)
    {
    ...
    // =========================================================================
    // 2. GIỮ NGUYÊN TOÀN BỘ DBSET CỦA QUÝ (KHÔNG ĐỤNG CHẠM)
    // =========================================================================
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<PointHistory>(entity =>
        {
            ...
            entity.HasOne(d => d.User).WithMany(p => p.PointHistories).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__Point_His__user___3F115E1A");
        });
        ...
        modelBuilder.Entity<RolePermission>(entity =>
        {
            ...
            entity.Property(e => e.CreatedAt).HasDefaultValueSql("(getdate())");
            entity.HasOne(d => d.Permission).WithMany(p => p.RolePermissions).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__Role_Perm__permi__10566F31");
            entity.HasOne(d => d.Role).WithMany(p => p.RolePermissions).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__Role_Perm__role___114A936A");
        });
        ...
        modelBuilder.Entity<RoomTypeAmenity>(entity =>
        {
            ...
            entity.Property(e => e.CreatedAt).HasDefaultValueSql("(getdate())");
            entity.HasOne(d => d.Amenity).WithMany(p => p.RoomTypeAmenities).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__RoomType___ameni__151B244E");
            entity.HasOne(d => d.RoomType).WithMany(p => p.RoomTypeAmenities).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__RoomType___room___160F4887");
        });
        ...
        modelBuilder.Entity<Shift>(entity =>
        {
            ...
            entity.Property(e => e.CreatedAt).HasDefaultValueSql("(getdate())");
            entity.Property(e => e.IsActive).HasDefaultValue(true);
            entity.HasOne(d => d.User).WithMany(p => p.Shifts).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__Shifts__user_id__625A9A57");
        });
    ...
    // =========================================================================
    // 4. HÀM GHI LOG THÔNG MINH - ĐÃ TÍCH HỢP BỘ LỌC CHỐNG SPAM
    // =========================================================================
    public override async Task<int> SaveChangesAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (_httpContextAccessor?.HttpContext?.User != null)
        {
            ...
            if (int.TryParse(userIdClaim, out int parsedId)) currentUserId = parsedId;
        }
        ...
        foreach (var entry in entries)
        {
            ...
            newEvents.Add(new
            {
                ...
                context = new { recordId = primaryKey },
                changes = new
                {
                    oldData = (entry.State == EntityState.Modified || entry.State == EntityState.Deleted) ? entry.OriginalValues.ToObject() : null,
                    newData = (entry.State == EntityState.Added || entry.State == EntityState.Modified) ? entry.CurrentValues.ToObject() : null
                },
                message = humanMessage
            });
        }

        // LƯU LOG GOM NHÓM VÀO DATABASE
        if (newEvents.Any())
        {
            var existingLog = AuditLogs.FirstOrDefault(l => l.UserId == currentUserId && l.CreatedAt >= today);

            if (existingLog != null)
            {
                try
                {
                    var jsonNode = JsonNode.Parse(existingLog.LogData)?.AsObject();
                    if (jsonNode != null && jsonNode["Events"] is JsonArray eventsArray)
                    {
                        foreach (var ev in newEvents) eventsArray.Add(JsonSerializer.SerializeToNode(ev));
                        jsonNode["TotalEvents"] = eventsArray.Count;
                        existingLog.LogData = jsonNode.ToJsonString();
                    }
                }
                catch
                {
                    var logDataObj = new { TotalEvents = newEvents.Count, Events = newEvents };
                    existingLog.LogData = JsonSerializer.Serialize(logDataObj);
                }
            }
            else
            {
                ...
                { 
                    ...
                    LogType = "Nghiệp vụ", 
                    LogData = JsonSerializer.Serialize(logDataObj), 
